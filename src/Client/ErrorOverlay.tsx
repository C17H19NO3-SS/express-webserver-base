...
const ErrorBoundaryApp = ({ logs }: { logs: any[] }) => {
  const errCount = logs.length;

  return (
    <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-12 sm:p-4 sm:items-center bg-white/20 sm:bg-white/40 sm:backdrop-blur-[2px] w-full h-full">
      <div className="bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] w-full max-w-2xl flex flex-col overflow-hidden border border-gray-200 ring-1 ring-black/5">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-white">
          ...
          <div className="flex items-center gap-4 text-[13px] font-medium text-gray-400">
            ...
            <button
              onClick={() => {
                const root = document.getElementById("ewb-error-root");
...
export function renderEWBErrorOverlay(logs: any[]) {
  const rootElement = document.getElementById("ewb-error-root");
  if (!rootElement) return;

  const win = window as any;
  if (!win._ewbErrorRoot) {
    win._ewbErrorRoot = createRoot(rootElement);
  }
  win._ewbErrorRoot.render(<ErrorBoundaryApp logs={logs} />);
}

// Mount the app to the DOM automatically on script load if there is an initial error
if (typeof window !== "undefined") {
  (window as any).renderEWBErrorOverlay = renderEWBErrorOverlay;
  const logs = (window as any).__EWB_ERROR_LOGS__ || [];
  if (logs.length > 0) {
    renderEWBErrorOverlay(logs);
  }
}
